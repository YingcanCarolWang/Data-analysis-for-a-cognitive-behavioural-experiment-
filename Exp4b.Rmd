...
rtsByWord <- rtWordByConSub %>% 
  ...
  summarise(
                   ...
                   sdRT = sd(adj, na.rm = TRUE),
                   seRT = sdRT / sqrt(N),
                   ci = qnorm(0.975)*sdRT/sqrt(N), 
                   ci_adjusted = ci*sqrt(6/5) 
                   )
...
Since the effect size of the competitor priming effect under current experimental design is smaller than what we expected, we decided to conduct a new power analysis to decide how many participants we should recruit for the other two experiments in this series.
