...
In order to examine why these two tasks resulted in different response patterns and tease apart their effects during competitor priming, we then pre-registered and conducted Exp4a (osf.io/9453v), which used PDT in the encoding training phase (when the prime is presented) and LDT in the recognition test phase (when the target is presented) separately, and each prime was repeatedly presented for 4 times. However, we only found a marginal trend similar to the results in Exp1-2. Here, in Exp4b, we will again use the LDT in both the encoding and recognition phases to determine whether the results from Exp4a were due to the change of tasks or the separate encoding/recognition phase design which lengthened the prime-target lag. In addition, we also tested participant's episodic memory of the words and pseudowords they heard during the test phase.
...
In the encoding phase, participants heard 108 different words (e.g. *hygeiene*) and pseudowords (e.g. *letto*) being repeated 4 times across 4 blocks.These items in encoding phase serve as primes that will perturb items to be presented in the recognition phase. We are only interested in general results patterns from the encoding phase.   
...
In the recognition phase, participants heard items that share the same initial segments with words and pseudowords presented in the encoding phase, such as *hijack*, shaing /haɪdʒ/ with *hygiene*. We would like to investigate whether this kind of word-primed target words evoke slower response as compared to unprimed words and examine the magnitude of the delay. 
